...
class BlueWitchGame extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    final spriteSheet = await images.load('blue_witch_idle.png');

    // Create the list of frames for vertical animation
    final List<Sprite> frames = [];
    const int frameCount = 6;  // Total number of frames (vertical)
    const double frameWidth = 28;  // Width of each frame
    const double frameHeight = 48;  // Height of each frame
    ...
    final blueWitch = SpriteAnimationComponent(
      ...
      size: Vector2(200, 200),  // Zoom to 600x600
      anchor: Anchor.center,  // Set anchor to center
      position: Vector2(size.x / 2, size.y / 2 - 50),  // Adjust position upwards
    );
